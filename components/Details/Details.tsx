    ...
    <section className="p-6 mb-12">
        ...
        {Data.map((item: any, index: any) => (
          <button
            ...
            className={`whitespace-nowrap rounded-3xl border-2 w-fit h-fit px-3 py-1 ${
              selected.uuid === item.uuid ? "bg-black text-white" : "border-black"
            }`}
          >
            {item.title}
          </button>
      ...
      <div className="flex justify-center flex-wrap lg:flex-nowrap mt-8 gap-16 px-5">
        <div className="w-full lg:w-2/6 2xl:w-1/6">
          {selected.image.data && (
            <Image src={getStrapiMediaUrl(selected.image.data?.attributes.url)} width={160} height={80} className="object-contain" alt="logo" />
          )}
