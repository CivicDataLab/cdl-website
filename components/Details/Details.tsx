...
const Details: React.FC<DetailsProps> = ({ data, content }) => {
  ...
  const [filter, setFilter] = useQueryState("filter", parseAsString.withDefault(filters[0].value));
        ...
        {selectedContent && (
          <div className="sm:max-w-52 sm:shrink-0 grow sticky top-0 h-fit">
            {selectedContent.image.data && (
              <Image src={getStrapiMediaUrl(selectedContent.image.data?.attributes.url)} alt="" width={120} height={120} />
            )}
        ...
        <div className="grid gap-6 grid-cols-[repeat(auto-fit,minmax(min(380px,100%),1fr))] grow">
          {data[filter] && data[filter].length > 0 ? (
            data[filter].map(({ id, attributes }) => (
              <a href={`${pathname}/${attributes.slug}`} key={id} className="h-fit w-full">
                <Image
